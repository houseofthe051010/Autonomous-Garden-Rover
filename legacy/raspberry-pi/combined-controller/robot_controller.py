...
def _find_repo_root():
    if "__file__" in globals():
        ...
        for candidate in source.parents:
            if (candidate / "README.md").is_file() and (candidate / "firmware").is_dir():
                return candidate
...
stm32_driver = _load_module(
    "rover_stm32_driver", "legacy/raspberry-pi/stm32-drive/main.py"
)
odesc_driver = _load_module(
    "rover_odesc_driver", "legacy/raspberry-pi/odesc/odesc_uart.py"
)
gd32_driver = _load_module(
    "rover_gd32_driver", "legacy/raspberry-pi/gd32-stepper/stepper_controller.py"
)
bno080_driver = _load_module(
    "rover_bno080_driver", "legacy/raspberry-pi/bno080/bno080_uart.py"
)
