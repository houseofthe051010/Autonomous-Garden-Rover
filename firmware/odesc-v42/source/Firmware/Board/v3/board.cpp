...
volatile uint32_t diagnostic_safety_fault_count = 0;

static bool pins_are_analog(GPIO_TypeDef* port, uint32_t pins) {
    for (uint32_t bit = 0; bit < 16; ++bit) {
        uint32_t pin = 1UL << bit;
        if ((pins & pin) && (((port->MODER >> (bit * 2)) & 0x3UL) != 0x3UL)) {
            return false;
        }
    }
    return true;
...
static bool pin_is_output(GPIO_TypeDef* port, uint32_t pin) {
    uint32_t bit = __builtin_ctz(pin);
    return ((port->MODER >> (bit * 2)) & 0x3UL) == 0x1UL;
}

void diagnostic_configure_safe_pins() {
    // Persisted configuration is untrusted in this image. Begin with every
    // exposed board GPIO as a high-impedance input.
    GPIO_InitTypeDef input = {};
    input.Mode = GPIO_MODE_INPUT;
    input.Pull = GPIO_NOPULL;
    input.Speed = GPIO_SPEED_FREQ_LOW;
    for (size_t i = 0; i < GPIO_COUNT; ++i) {
        if (gpios[i]) {
            input.Pin = gpios[i].pin_mask_;
            HAL_GPIO_Init(gpios[i].port_, &input);
        }
    }
    ...
    // ADC1 channels 0..7 plus all M0 high-side phase controls.
    gpio.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 |
               GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7 |
               GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10;
    HAL_GPIO_Init(GPIOA, &gpio);

    // ADC1 channels 8/9, AUX brake outputs, and M0 low-side controls.
    gpio.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_10 | GPIO_PIN_11 |
               GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15;
    HAL_GPIO_Init(GPIOB, &gpio);

    // ADC1 channels 10..15 plus the unused M1 high-side phase controls.
    gpio.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 |
               GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7 |
    ...
    gate.Pin = EN_GATE_Pin;
    gate.Mode = GPIO_MODE_OUTPUT_PP;
    gate.Pull = GPIO_PULLDOWN;
    gate.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_WritePin(EN_GATE_GPIO_Port, EN_GATE_Pin, GPIO_PIN_RESET);
    HAL_GPIO_Init(EN_GATE_GPIO_Port, &gate);
}

void diagnostic_force_safe_outputs() {
    // Disable the DRV8301 first. This is independent of timer/PWM state.
    EN_GATE_GPIO_Port->BSRR = (uint32_t)EN_GATE_Pin << 16U;

    // Stop and disconnect both advanced motor-control timers.
    ...
    // The ADC diagnostic never drives the AUX/brake half bridge.
    TIM2->CCER = 0;
    TIM2->CR1 &= ~TIM_CR1_CEN;
    TIM2->CCR3 = 0;
    TIM2->CCR4 = TIM2->ARR + 1UL;
}

uint32_t diagnostic_safety_status() {
    uint32_t status = 0;
    status |= ((EN_GATE_GPIO_Port->ODR & EN_GATE_Pin) == 0) ? (1UL << 0) : 0;
    status |= ((TIM1->BDTR & (TIM_BDTR_MOE | TIM_BDTR_AOE)) == 0) ? (1UL << 1) : 0;
    status |= ((TIM8->BDTR & (TIM_BDTR_MOE | TIM_BDTR_AOE)) == 0) ? (1UL << 2) : 0;
    status |= ((TIM1->CR1 & TIM_CR1_CEN) == 0) ? (1UL << 3) : 0;
    status |= ((TIM8->CR1 & TIM_CR1_CEN) == 0) ? (1UL << 4) : 0;
    status |= ((TIM2->CR1 & TIM_CR1_CEN) == 0) ? (1UL << 5) : 0;
    status |= (TIM1->CCER == 0) ? (1UL << 6) : 0;
    status |= (TIM8->CCER == 0) ? (1UL << 7) : 0;
    status |= (TIM2->CCER == 0) ? (1UL << 8) : 0;

    const uint32_t gpioa_safe = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 |
                                GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7 |
                                GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10;
    const uint32_t gpiob_safe = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_10 | GPIO_PIN_11 |
                                GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15;
    const uint32_t gpioc_safe = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 |
                                GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7 |
                                GPIO_PIN_8 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12;
    bool pins_safe = pins_are_analog(GPIOA, gpioa_safe)
                  && pins_are_analog(GPIOB, gpiob_safe)
                  && pins_are_analog(GPIOC, gpioc_safe);
    status |= pins_safe ? (1UL << 9) : 0;
    status |= pin_is_output(EN_GATE_GPIO_Port, EN_GATE_Pin) ? (1UL << 10) : 0;
    bool chip_selects_safe = pin_is_output(M0_nCS_GPIO_Port, M0_nCS_Pin)
                          && pin_is_output(M1_nCS_GPIO_Port, M1_nCS_Pin)
                          && ((M0_nCS_GPIO_Port->ODR & M0_nCS_Pin) != 0)
                          && ((M1_nCS_GPIO_Port->ODR & M1_nCS_Pin) != 0);
    status |= chip_selects_safe ? (1UL << 11) : 0;
    return status;
}
#endif
