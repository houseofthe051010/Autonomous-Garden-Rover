...
// Flipsky/SEQURE ODESC V4.2 56V uses the 11:1 voltage-sense divider.
...
// All bits must remain set while the read-only ADC diagnostic is running.
#define ODESC_DIAGNOSTIC_SAFE_STATUS_ALL ((1UL << 12) - 1UL)
void diagnostic_configure_safe_pins();
void diagnostic_force_safe_outputs();
uint32_t diagnostic_safety_status();
extern volatile uint32_t diagnostic_safety_fault_count;
#endif
