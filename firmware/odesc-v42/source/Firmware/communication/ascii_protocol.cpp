...
#include "communication/uart_blackbox.hpp"
...
#if ODESC_ADC_DIAGNOSTIC
    switch(cmd[0]) {
        case 'a': cmd_adc_diagnostic(cmd, use_checksum);              break;
        case 'r': cmd_read_property(cmd, use_checksum);               break;
        case 'h': cmd_help(cmd, use_checksum);                        break;
        case 'i': cmd_info_dump(cmd, use_checksum);                   break;
        default:  respond(use_checksum, "ADC diagnostic is read only"); break;
    }
#else
    switch(cmd[0]) {
        ...
        case 'd': cmd_uart_diagnostic(cmd, use_checksum);             break;  // UART diagnostics
        default : cmd_unknown(nullptr, use_checksum);                 break;
    }
#endif
}

void AsciiProtocol::cmd_uart_diagnostic(char *pStr, bool use_checksum) {
    if (pStr[1] == 's') {
        respond(use_checksum, "DS %lu %lu %lu %lu %lu %lu",
                (unsigned long)uart_blackbox_record_count(),
                (unsigned long)uart_blackbox_rx_bytes(),
                (unsigned long)uart_blackbox_dma_restarts(),
                (unsigned long)uart_blackbox_error_events(),
                (unsigned long)uart_blackbox_silence_events(),
                (unsigned long)uart_tx_queue_drop_count());
        return;
    }
    unsigned index = 0;
    UartBlackboxRecord record = {};
    if (sscanf(pStr, "da %u", &index) == 1 &&
        uart_blackbox_read(index, &record)) {
        respond(use_checksum, "DA %x %lx %lx %lx %lx", index,
                (unsigned long)record.sequence, (unsigned long)record.uptime_ms,
                (unsigned long)record.reason, (unsigned long)record.hal_error,
                (unsigned long)record.uart_status);
        return;
    }
    if (sscanf(pStr, "db %u", &index) == 1 &&
        uart_blackbox_read(index, &record)) {
        respond(use_checksum, "DB %x %lx %lx %lx %lx", index,
                (unsigned long)record.rx_state,
                (unsigned long)record.dma_remaining,
                (unsigned long)record.rx_bytes,
                (unsigned long)record.dma_restarts);
        return;
    }
    if (sscanf(pStr, "dc %u", &index) == 1 &&
        uart_blackbox_read(index, &record)) {
        respond(use_checksum, "DC %x %lx %lx %lx %lx", index,
                (unsigned long)record.axis_error,
                (unsigned long)record.motor_error,
                (unsigned long)record.estimator_error,
                (unsigned long)record.axis_state);
        return;
    }
    if (sscanf(pStr, "dd %u", &index) == 1 &&
        uart_blackbox_read(index, &record)) {
        respond(use_checksum, "DD %x %.3f %lx", index,
                (double)record.velocity_turns_s,
                (unsigned long)record.crc32);
        return;
    }
    respond(use_checksum, "usage: ds | da/db/dc/dd index");
}

#if ODESC_ADC_DIAGNOSTIC
void AsciiProtocol::cmd_adc_diagnostic(char* pStr, bool use_checksum) {
    if (pStr[1] == 's') {
        respond(use_checksum, "SAFE 0x%03lx faults %lu",
                (unsigned long)diagnostic_safety_status(),
                (unsigned long)diagnostic_safety_fault_count);
        return;
    }

    unsigned channel = ADC_CHANNEL_COUNT;
    if ((sscanf(pStr, "a %u", &channel) != 1) || channel >= ADC_CHANNEL_COUNT) {
        respond(use_checksum, "usage: a 0..15 or astatus");
        return;
    }

    uint16_t raw = adc_measurements_[channel];
    unsigned millivolts = ((unsigned)raw * 3300UL + 2048UL) / 4096UL;
    respond(use_checksum, "ADC %u %u %u", channel, raw, millivolts);
}
#endif
