...
// The 8 kHz RX poll and UART TX completion originally shared a four-entry
// queue. If that queue was full in the TX ISR, the only completion event was
// lost and AsyncStreamSink remained busy until the board was power-cycled.
// Latch completion independently so any later UART-thread event can service it.
static volatile bool uart_tx_complete_pending = false;
static volatile uint32_t uart_tx_queue_drops = 0;
...
static void service_uart_tx_completion() {
    if (!uart_tx_complete_pending) return;
    uart_tx_complete_pending = false;
...
static void uart_server_thread(void * ctx) {
    ...
    for (;;) {
        ...
        // This also recovers a TX-complete notification that could not be
        // inserted by the ISR because the shared event queue was full.
        service_uart_tx_completion();
        ...
        switch (event.value.v) {
            case 1: {
                ...
                if (huart_->RxState != HAL_UART_STATE_BUSY_RX) {
                    uart_blackbox_note_dma_restart(huart_);
                    HAL_UART_AbortReceive(huart_);
                    // Reading SR then DR clears PE/FE/NE/ORE on STM32F4.
                ...
                if (new_rcv_idx < dma_last_rcv_idx) {
                    uart_blackbox_note_rx(UART_RX_BUFFER_SIZE - dma_last_rcv_idx);
                    uart_rx_stream.did_receive(dma_rx_buffer + dma_last_rcv_idx,
                            UART_RX_BUFFER_SIZE - dma_last_rcv_idx);
                    dma_last_rcv_idx = 0;
                }
                if (new_rcv_idx > dma_last_rcv_idx) {
                    uart_blackbox_note_rx(new_rcv_idx - dma_last_rcv_idx);
                    uart_rx_stream.did_receive(dma_rx_buffer + dma_last_rcv_idx,
                            new_rcv_idx - dma_last_rcv_idx);
                    dma_last_rcv_idx = new_rcv_idx;
                }
                uart_blackbox_poll(huart_);
            } break;

            case 2: {
                // Completion was serviced above using the independently
                // latched flag. A stale queue entry is intentionally a no-op.
            } break;
...
uint32_t uart_tx_queue_drop_count(void) {
    return uart_tx_queue_drops;
}
