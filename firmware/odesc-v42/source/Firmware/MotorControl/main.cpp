...
void ODrive::do_fast_checks() {
    if (vbus_voltage_external_fault) {
        disarm_with_error(ERROR_EXTERNAL_VBUS_INVALID);
        return;
    }
...
static void rtos_main(void*) {
    ...
    // USB is the only intended diagnostic transport. No gate-driver setup,
    // motor PWM, brake PWM, analog control thread, or axis task is started.
    init_communication();
    for (;;) {
        if (diagnostic_safety_status() != ODESC_DIAGNOSTIC_SAFE_STATUS_ALL) {
            ++diagnostic_safety_fault_count;
        }
        diagnostic_force_safe_outputs();
        osDelay(1);
    }
#else
    //osDelay(100);
...
extern "C" int main(void) {
...
#if ODESC_ADC_DIAGNOSTIC
    // Ignore persisted GPIO modes. Every scanned channel is analog, every other
    // external GPIO is high impedance, and power controls are disconnected.
    diagnostic_configure_safe_pins();
    diagnostic_force_safe_outputs();
#endif
    ...
    osMessageQDef(uart_event_queue, 8, uint32_t);
