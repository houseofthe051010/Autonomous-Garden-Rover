...
void Motor::current_meas_cb(uint32_t timestamp, std::optional<Iph_ABC_t> current) {
    ...
    if (armed_state_ == 1 || armed_state_ == 2) {
        ...
        if (!current.has_value() && is_armed_) {
            error_ |= ERROR_CURRENT_MEASUREMENT_UNAVAILABLE;
        }
...
void Motor::dc_calib_cb(uint32_t timestamp, std::optional<Iph_ABC_t> current) {
    ...
    // This clone's nominal zero-vector sample contains phase current once PWM
    // is active. Keep the bounds-checked offset measured during IDLE fixed for
    // the armed interval; resume continuous calibration as soon as it disarms.
    if (is_armed_) {
        return;
    }
