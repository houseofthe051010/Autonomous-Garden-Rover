...
float vbus_voltage_external_scale = 19.231321981f;
...
namespace {
constexpr uint32_t external_vbus_gpio = 3;
constexpr uint16_t external_vbus_adc_channel = 2; // v3.6 GPIO3 is PA2/ADC1_IN2
constexpr float external_vbus_minimum_voltage = 1.0f;
constexpr float external_vbus_maximum_adc_voltage = 3.25f;
constexpr uint32_t external_vbus_status_warmup = 1U << 0;
constexpr uint32_t external_vbus_status_gpio_mode = 1U << 1;
constexpr uint32_t external_vbus_status_low = 1U << 2;
constexpr uint32_t external_vbus_status_high = 1U << 3;
constexpr uint32_t external_vbus_status_mismatch = 1U << 4;

void validate_external_vbus();
}
...
void vbus_sense_adc_cb(uint32_t adc_value) {
    ...
    static float filtered_external_voltage = 0.0f;
    static uint32_t raw_overvoltage_samples = 0;
    const uint16_t external_raw = adc_measurements_[external_vbus_adc_channel];
    vbus_voltage_external_adc = external_raw * adc_ref_voltage / adc_full_scale;
    const float raw_external_voltage =
        vbus_voltage_external_adc * vbus_voltage_external_scale;
    if (!(filtered_external_voltage > 0.0f)) {
        filtered_external_voltage = raw_external_voltage;
    } else {
        // Suppress isolated GPIO3 ADC outliers without hiding a real bus rise.
        filtered_external_voltage +=
            0.10f * (raw_external_voltage - filtered_external_voltage);
    }
    vbus_voltage_external = filtered_external_voltage;

    // Open/short/ADC-overrange faults remain synchronous with motor control.
    // Multi-sample plausibility checks run in the analog task below so they
    // cannot delay the phase-current interrupt during PWM arming.
    if (!(vbus_voltage_external >= external_vbus_minimum_voltage)) {
        vbus_voltage_external_valid = false;
        vbus_voltage_external_fault = true;
        vbus_voltage_external_status = external_vbus_status_low;
        vbus_voltage = NAN;
    } else if (!(vbus_voltage_external_adc < external_vbus_maximum_adc_voltage)) {
        vbus_voltage_external_valid = false;
        vbus_voltage_external_fault = true;
        vbus_voltage_external_status = external_vbus_status_high;
        vbus_voltage = NAN;
    } else if (vbus_voltage_external_valid && !vbus_voltage_external_fault) {
        vbus_voltage = vbus_voltage_external;
    } else if (vbus_voltage_external_fault) {
        vbus_voltage = NAN;
    } else {
        vbus_voltage = vbus_voltage_internal;
    }

    // The filtered control value is stable, but a real overvoltage must still
    // disarm promptly. Three consecutive raw samples reject one-off ADC noise
    // and trip in less than 0.5 ms at the 8 kHz control rate.
    if (raw_external_voltage > odrv.config_.dc_bus_overvoltage_trip_level) {
        if (raw_overvoltage_samples < 3) ++raw_overvoltage_samples;
        if (raw_overvoltage_samples >= 3) vbus_voltage = raw_external_voltage;
    } else {
        raw_overvoltage_samples = 0;
    }
}
...
void validate_external_vbus() {
    ...
    constexpr uint32_t valid_samples_required = 3;
    constexpr uint32_t invalid_samples_required = 3;
    static uint32_t valid_samples = 0;
    static uint32_t invalid_samples = 0;
    static bool accepted_once = false;

    uint32_t status = 0;
    if (odrv.config_.gpio_modes[external_vbus_gpio] !=
        ODriveIntf::GPIO_MODE_ANALOG_IN) {
        status |= external_vbus_status_gpio_mode;
    }
    if (!(vbus_voltage_external >= external_vbus_minimum_voltage)) {
        status |= external_vbus_status_low;
    }
    if (!(vbus_voltage_external_adc < external_vbus_maximum_adc_voltage)) {
        status |= external_vbus_status_high;
    }
    const float internal_tolerance =
        std::max(1.0f, 0.03f * vbus_voltage_internal);
    if (vbus_voltage_external + internal_tolerance < vbus_voltage_internal ||
        (vbus_voltage_internal < internal_compare_ceiling &&
         vbus_voltage_external > vbus_voltage_internal + internal_tolerance)) {
        status |= external_vbus_status_mismatch;
    }
    ...
    CRITICAL_SECTION() {
        if (status == 0) {
            ...
            if (valid_samples >= valid_samples_required) {
                ...
                vbus_voltage_external_valid = false;
                vbus_voltage_external_status = external_vbus_status_warmup;
            }
        } else {
            valid_samples = 0;
